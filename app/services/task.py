...
# commit-ы происходят тут, потому что сервис отправляет несколько запросов в БД и
# если бы это происходило в модуле repositories то сессия ///сбрасывалась///? бы
class TaskService:
    ...
    def list_tasks(self) -> list[Task]:
        # в этой переменной находится результат выполнения функции
        # get_all - ЭТО ВСЕ ТАСКИ
        tasks_orm = self.task_repository.get_all()
        # возвращаем список таск схем, при помощи model_validate этот объект
        # гарантированно валидный но, чтобы это работало нам нужно прописать
        # в нашей схеме model_config, чтобы она была словарем потому что этот
        # метод работает только со словарями
    ...
    def create_task(self, payload: TaskCreate) -> Task:
        # вызываем метод create, в нем создается новая запись в сессии, все поля
        # заполняются так как по умолчанию completed=False, а if - primary кей и
        #  генерится по дефолту передаем только тайтл
    ...
    # метод обновления таски, получаем id таски от фронтенда и передаем тело
    # запроса на обнову возвращаем таску отредаченную
    def update_task(self, task_id: str, payload: TaskUpdate) -> Task:
        ...
        # если были внесены изменения хоть в одно поле, то оно меняется в сессии и
        # потом комит в БД
        ...
        # комит в БД
        self.db.commit()
        # возвращем обновленнуб БД в валидном виде,
        # благодаря функции model_validate из Pydantic
    ...
    def delete_task(self, task_id: str) -> None:
        ...
        if not task_for_delete:
            raise TaskNotFound(f"Задача с id {task_id} не найдена")
        # вызываем метод удаления таски из репозитория
        # (он не выполняет комит, все комиты тут)
