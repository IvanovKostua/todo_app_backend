...
class CategoryRepository:
    ...
    #получаем список CategoryORM (метод scalars возвращает список запрашиваемых полей из таблицы),
    #тут указана вся запись целиком, потому он вернет список категорий
    def get_all(self) -> list[CategoryORM]:
        return self.db.scalars(select(CategoryORM))
    ...
    #тут записываем в переменную новую категорию, поля заполняем в соответствии с переданными значениями
    #записываем title а id у нас primary_key и в лямбда функции создается
    ...
    #принимает категорию, удаляем ее из сессии, в дальнейшем коммитим и она удаляется уже из БД
