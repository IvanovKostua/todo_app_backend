...
class TaskRepository:
    ...
    #получаем список TaskORM (метод scalars возвращает список запрашиваемых полей из таблицы),
    #тут указана вся запись целиком, потому он вернет список задач
    ...
    #тут записываем в переменную новую таску, поля заполняем в соответствии с переданными значениями
    #записываем title, completed в начале всегда false, а id у нас primary_key и в лямбда функции создается
    ...
    #принимает таску, удаляем ее из сессии, в дальнейшем коммитим и она удаляется уже из БД
