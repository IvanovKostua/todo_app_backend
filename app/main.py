...
from app.core.config import (
    get_settings,
)  # функция, возвращающая все настройки, в том числе cors_allow_origins
...
# корс определяем, чтобы у fatapi было понимание, кто может подключаться и
# какие запросы, с какими заголовкам отправлять нашему серверу
# (не рекомендуется ставиь в методс и хэдэр * на продакшне)
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
