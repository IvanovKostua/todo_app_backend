...
# прописываем конкретные роуты (в скобках уже не будет /tasks,
# потому что указали префикс)
...
def get_tasks(task_service: TaskService = Depends(get_task_service)) -> list[Task]:
    ...
    # тут вызываем метод для получения всех тасок, который вызывает
    # аналогичный метод уже в repositories
